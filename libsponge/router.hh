...
class Router {
    struct route_rule{
        uint8_t prefix_length{0}; //掩码位数
        uint32_t route_prefix{0}; //ipv4地址
        size_t interface_num{0}; //对应接口id
        std::optional<Address> next_hop{std::nullopt}; //下一跳目的地
    };
    //! The router's collection of network interfaces
    std::vector<AsyncNetworkInterface> _interfaces{};
    std::vector<route_rule> _router_table{};
    //! Send a single datagram from the appropriate outbound interface to the next hop,
    //! as specified by the route with the longest prefix_length that matches the
    //! datagram's destination address.
    void route_one_datagram(InternetDatagram &dgram);
    bool prefix_equal(uint32_t pre_a,uint32_t pre_b,uint8_t mask_bit);
  public:
