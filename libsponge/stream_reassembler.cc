...
void StreamReassembler::_insert_waiting(const seg& new_seg){
    if (_waiting.empty()) //为空直接插入
    {
        _waiting.insert(new_seg);
...
void StreamReassembler::push_substring(const std::string &data, const uint64_t index, const bool eof) {
    ...
    {   
        auto tmp_string = new_seg.data.substr(_first_unassembled - index);
...
END:
    ...
    if (_eof_flag && _output.bytes_written() == _eof_pos)//收到关闭信号且最后一个字节已写入（可能有丢弃的所以没有全写入）
        _output.end_input();
    std::cout<<"bytes_unass:"<<_bytes_unassembled<<" bytes_written:"<<_output.bytes_written()<<" eof_pos:"<<_eof_pos<<std::endl;
    return;
}
